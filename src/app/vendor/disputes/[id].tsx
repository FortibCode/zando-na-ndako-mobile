...
import {
  fetchVendeurLitigeDetail, envoyerMessageLitige, uploaderPreuveLitige,
...
export default function VendorDisputeDetailScreen() {
  ...
  const load = useCallback(async (opts?: { silent?: boolean }) => {
    ...
    try {
      const data = await fetchVendeurLitigeDetail(id);
      setLitige(data);
  ...
  return (
    <SafeAreaView style={[styles.screen, { backgroundColor: colors.background }]}>
      ...
      <Animated.View entering={FadeInDown.duration(300).springify()} style={[styles.header, { backgroundColor: colors.surface, borderBottomColor: colors.border }]}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={[styles.title, { color: colors.text }]}>{litige?.numero || t('vendorDisputeDetail.title', 'Litige')}</Text>
          {litige && (
            ...
            {(litige?.remboursements ?? []).map((r) => (
              <View key={r.id} style={[styles.systemCard, { backgroundColor: colors.freshSoft }]}>
                ...
                <Text style={[styles.systemText, { color: colors.success }]}>
                  {t('vendorDisputeDetail.refund', 'Remboursement')} : {Math.round(Number(r.montant)).toLocaleString('fr-FR')} {r.devise} — {r.statut}
                </Text>
              </View>
            ))}

            {messages.map((m, i) => {
              const mine = m.sender_type === 'vendeur';
              const isSystem = m.sender_type === 'system';
              const senderLabel = m.sender_type === 'admin' ? t('vendorDisputeDetail.adminLabel', 'Administration') : m.sender_type === 'client' ? t('vendorDisputeDetail.clientLabel', 'Client') : '';
              if (isSystem) {
                return (
          ...
          {!closed ? (
            <Animated.View entering={FadeInUp.duration(300).delay(150).springify()} style={[styles.inputRow, { backgroundColor: colors.surface, borderTopColor: colors.border }]}>
              ...
              <TextInput
                ...
                placeholder={t('vendorDisputeDetail.messagePlaceholder', 'Écrire un message...')}
            ...
            <View style={[styles.closedBar, { backgroundColor: colors.surfaceAlt, borderTopColor: colors.border }]}>
              <Text style={[styles.closedText, { color: colors.textSecondary }]}>
                {t('vendorDisputeDetail.closed', 'Ce litige est clôturé.')}
              </Text>
            </View>
          )}
