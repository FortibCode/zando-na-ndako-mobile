  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert(t('changePassword.success', 'Mot de passe changé'), t('changePassword.successDesc', 'Veuillez vous reconnecter avec votre nouveau mot de passe.'), [
        { text: 'OK', onPress: () => { clearAuthToken(); router.replace('/auth'); } },
      ]);
    } catch (e: any) {
  ...
  return (
    <SafeAreaView style={[styles.screen, { backgroundColor: colors.background }]}>
      ...
      <Animated.View entering={FadeInDown.duration(300).springify()} style={[styles.header, { backgroundColor: colors.surface, borderBottomColor: colors.surfaceBorder }]}>
        ...
        <Text style={[styles.title, { color: colors.primary }]}>{t('changePassword.title', 'Changer mon mot de passe')}</Text>
      </Animated.View>

      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <Animated.View entering={FadeInUp.duration(400).delay(60).springify()} style={[styles.iconWrap, { backgroundColor: colors.primaryLight }]}>
          <Lock color={colors.primary} size={28} />
        </Animated.View>

        <Animated.View entering={FadeInUp.duration(400).delay(120).springify()}>
          <Text style={[styles.label, { color: colors.primary }]}>{t('changePassword.current', 'Mot de passe actuel')}</Text>
          <TextInput
            value={ancien}
            ...
            style={[styles.field, { borderColor: colors.surfaceBorder, backgroundColor: colors.surface, color: colors.text }]}
          />
        </Animated.View>

        <Animated.View entering={FadeInUp.duration(400).delay(160).springify()}>
          <Text style={[styles.label, { color: colors.primary }]}>{t('changePassword.newPassword', 'Nouveau mot de passe')}</Text>
          <TextInput
            value={nouveau}
        ...
        <Animated.View entering={FadeInUp.duration(400).delay(200).springify()}>
          <Text style={[styles.label, { color: colors.primary }]}>{t('changePassword.confirmNew', 'Confirmer le nouveau mot de passe')}</Text>
          <TextInput
            value={confirmation}
        ...
        <Animated.View entering={FadeInUp.duration(400).delay(260).springify()}>
          ...
          >
            {submitting ? <ActivityIndicator color="#FFF" /> : <Text style={styles.submitBtnText}>{t('changePassword.submit', 'Changer le mot de passe')}</Text>}
          </Pressable>
        </Animated.View>
      </ScrollView>
    </SafeAreaView>
