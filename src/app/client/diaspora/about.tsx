...
function InfoSection({
  ...
  return (
    <Animated.View entering={FadeInUp.duration(400).delay(delay).springify()} style={[styles.section, { backgroundColor: tint }]}>
      <Text style={styles.sectionTitle}>{title}</Text>
      {items.map(({ icon: Icon, text }, i) => (
        <View key={text} style={[styles.itemRow, i < items.length - 1 && styles.itemRowBorder]}>
  ...
  return (
    <SafeAreaView style={styles.screen}>
      ...
      <Animated.View entering={FadeInDown.duration(300).springify()} style={styles.header}>
        ...
        <Text style={styles.title}>{t('diaspora.about.title', 'Mode Diaspora')}</Text>
      </Animated.View>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <Animated.View entering={FadeInUp.duration(400).delay(260).springify()} style={[styles.section, { backgroundColor: '#EEF4FF' }]}>
          <Text style={styles.sectionTitle}>{t('diaspora.about.supportTitle', 'SUPPORT DISPONIBLE')}</Text>
          {SUPPORT.map(({ icon: Icon, label, action }, i) => (
            <Pressable
              key={label}
              onPress={action}
              style={[styles.itemRow, i < SUPPORT.length - 1 && styles.itemRowBorder]}
            >
          ...
          <Text style={styles.helpTagline}>{t('diaspora.about.helpTagline', 'Nous sommes là pour vous aider !')}</Text>
        </Animated.View>

        <Animated.View entering={FadeInUp.duration(400).delay(340).springify()}>
          <Pressable onPress={() => router.push('/client/diaspora' as any)} style={styles.cta}>
            <Text style={styles.ctaText}>{t('diaspora.about.cta', 'Commencer un envoi')}</Text>
          </Pressable>
        </Animated.View>
      </ScrollView>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { color: BLUE, fontSize: 13, fontWeight: '900', letterSpacing: 0.6, marginBottom: 10 },
  itemRow: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 11 },
  itemRowBorder: { borderBottomWidth: 1, borderBottomColor: 'rgba(15,23,42,0.06)' },
  itemIcon: {
    width: 34, height: 34, borderRadius: 11,
    alignItems: 'center', justifyContent: 'center',
  },
  itemText: { color: '#334155', fontSize: 13.5, fontWeight: '600', flex: 1, lineHeight: 19 },
  helpTagline: { color: RED, fontSize: 14, fontWeight: '900', textAlign: 'center', marginTop: 8 },
